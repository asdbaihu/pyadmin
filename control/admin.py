		...
		if len(ps) == 0:
			...
			if int(ps[0][2]) == 2:
				...
				if 'table' in post:
					if post['table'] !="":
						table = post['table']
						cur.execute("select count(*) from information_schema.tables where table_name='%s' "%post['table'])
						check = cur.fetchone()
						if check[0] > 0:
							...
							if countrows[0] == 0: # no records in table, table is empty
								cur.execute("select column_name, data_type from information_schema.columns where table_name = '" + post['table'] + "'")
								rws = cur.fetchall()
								...
								if check5[0] > 0:
									...
									if 'movecols' in post:
										if len(post.getall('movecols')) > 0:
											...
											cur.execute("select " + ",".join(cols) + " from (" + check3[0] + ") as jotikaandmendaka limit 1")
											que =  check3[0]
											colslist = [desc[0] for desc in cur.description]
											filcols = [fils for fils in cols if fils not in hidefils]
				...
				for fil in filcols:
					...
					if types[fil] == 'int':
						...
						grofil += fil.title().replace("_"," ") + """ ilike <input class="input-mini" name='fil""" + fil + """' value=''/>|| """
						grofil += fil.title().replace("_"," ") + """ not ilike <input class="input-mini" name='notfil""" + fil + """' value=''/>|| """
...
<div class="btn-group">
  <button data-toggle="dropdown" class="btn dropdown-toggle"  data-placeholder="Move columns">
    ...
    <ul id="sortable6" class="connectedSortable dropdown-menu">"""%(",".join(orderby), by ,",".join(hidecols),",".join(hidefils),movecols,table,ops)
				for colsname in colslist:
		...
		$parent.find('button[name=load]').click(function () {
		  $.ajax({
			url: """%(table,str(cols)) + loadurl + ""","""
				page += """	data: JSON.parse(JSON.stringify({types:%s,"display":%s,table:"%s",cols:%s,orderby:%s,by:"%s",filcols:%s%s%s,que:"%s"})),"""%(str(types),display,table,str(cols),str(orderby),by,str(filcols),send_data,movecols,re.sub('\s+',' ',que.replace('"','\\"')))
				page += """	dataType: 'json',
							type: 'POST',					
				success: function (res) {
					var data = [], row;
			  ...
			  $(".page2").html("<strong>Page 1/ "  + Math.round(res.sum_page)+"</strong> ::::" + res.test);
				...
				page += """ 		data: JSON.parse(JSON.stringify({types:%s,"page":num,"display":%s,table:"%s",cols:%s,orderby:%s,by:"%s",filcols:%s%s%s,que:"%s"})),"""%(str(types),display,table,str(cols),str(orderby),by,str(filcols),send_data,movecols,re.sub('\s+',' ',que.replace('"','\\"')))
				page += """			dataType: 'json',
									type: 'POST',
									success: function (res) {
					var data = [], row;
