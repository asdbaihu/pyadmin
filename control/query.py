		...
		if len(ps) == 0:
			...
			if int(ps[0][2]) == 2:
				...
				if 'table' in post:
					if post['table'] !='':
						table = post['table']
						cur.execute("select query from settings where tablename='%s'"%(table.replace("'","''")))
						check = cur.fetchone()
						if check == None:
				...
				if 'movecols' in post:
					if len(post.getall('movecols')) != 0:
						...
						if check[0] !="":
							cur.execute("select " + ",".join(cols) + " from " + " from ".join(post["query"].split("from")[1:]))
						else:
					...
					movecols  =""
				cur.execute("select tablename,query from settings where edit='no' order by id")
				lst = cur.fetchall()
				ops =""
				for l in lst:
				...
				if 'table' in post:
					if post['table'] !='':
						...
						if check == None:
							...
							cur.execute(check[0].replace("*", ",".join(cols) + " "))
					else:
						cur.execute("select  " + ",".join(cols) + "  from %s limit 1"%table)
				else:
					cur.execute("select")
		...
		$parent.find('button[name=load]').click(function () {
		  $.ajax({
			url: """%(table,str(cols)) + loadurl + ""","""
				page += """	data: {types:%s,"display":%s,table:"%s",cols:%s,orderby:%s,by:"%s",filcols:%s%s%s,query:"%s"},"""%(str(types),display,table,str(cols),str(orderby),by,str(filcols),send_data,movecols,check[0])
				page += """	dataType: 'json',
							type: 'POST',					
				success: function (res) {
					var data = [], row;
			  ...
			  $(".page2").html("<strong>Page 1/ "  + Math.round(res.sum_page)+"</strong> ::::" + res.test);
				...
				page += """ 		data: {types:%s,"page":num,"display":%s,table:"%s",cols:%s,orderby:%s,by:"%s",filcols:%s%s%s,query:"%s"},"""%(str(types),display,table,str(cols),str(orderby),by,str(filcols),send_data,movecols,check[0])
				page += """			dataType: 'json',
									type: 'POST',
									success: function (res) {
					var data = [], row;
