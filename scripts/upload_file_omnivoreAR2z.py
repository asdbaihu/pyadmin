...
agent text, module text,domain text,title text,link text, quest text, answer text, time timestamp,process_time numeric,img_id text,
					...
					try:
						cur.copy_expert("""copy omnivore_%s_%s_%s(agent , module ,domain ,title ,link , quest , answer , time) from '/tmp/file_upload/%s%s%s' delimiter ';' CSV HEADER escape '\\' quote '"' """%(year,month,day,fn,account,table_name),sys.stdout)
