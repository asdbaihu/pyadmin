...
agent text, module text,domain text,title text,link text, quest text, answer text, time timestamp,update_time timestamp default now() )"""%(table))
					try:
						...
						db.execute("""copy %s(agent, module,domain,title,link, quest, answer, time) from '/tmp/file_upload/%s' delimiter ';' CSV HEADER escape '\\' quote '"' """%(table,fn))
