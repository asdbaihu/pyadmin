...
no text,agent text, module text,domain text,title text,link text, quest text, answer text, time timestamp, process_time numeric,img_id text,
					...
					# page += """copy qc_project_csv%s%s(no ,agent , module ,domain ,title ,link , quest , answer , time, process_time) from '/usr/local/www/apache24/wsgi-scripts/file_upload/%s%s%s_%s_%s_%s_%s_%s' delimiter ';' CSV HEADER escape '\\' quote '"' """%(year,month,fn,account,year,month,day,hour,minute,second)

								
					try:
						cur.copy_expert("""copy omnivore_%s_%s_%s(no ,agent , module ,domain ,title ,link , quest , answer , time , process_time ) from '/tmp/file_upload/%s%s%s' delimiter ';' CSV HEADER escape '\\' quote '"' """%(year,month,day,fn,account,table_name),sys.stdout)
