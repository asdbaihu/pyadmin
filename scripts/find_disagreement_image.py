...
for table_name in table_names:
    ...
    cur.execute("""  insert into master_table_%s(id,agent,link,title,customize,variant_check,correct_check,status,price,currency,condition,availability,cds_key,img_data,image,update_time) select id,agent,link,title,customize,variant_check,correct_check,status,price,currency,condition,availability,cds_key,img_data, image,update_time from %s """%(today,table_name[0]))
    conn.commit()
    cur.close()
    conn.close()
...
cur = conn.cursor()
cur.execute("""
create table disagreement_%s as select e.id,e.agent,e.link,e.title,e.customize,e.variant_check,e.correct_check,e.status,e.price,e.currency,e.condition,e.availability,e.cds_key,e.img_data,e.image,e.update_time from (select a.cds_key,a.title,a.count from
...
conn = getConnection()
cur = conn.cursor()
cur.execute("select id,agent,link,title,customize,variant_check,correct_check,status,price,currency,condition,availability,cds_key,img_data,image,update_time from disagreement_%s"%today)
