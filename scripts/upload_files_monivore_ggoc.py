...
no text,agent text, module text,domain text,title text,link text, quest text, answer text, time timestamp, process_time numeric,img_id  text ,update_time timestamp default now() )"""%(table))
					try:
						...
						db.execute("""copy %s(no,agent, module,domain,title,link, quest, answer, time, process_time,img_id) from '/tmp/file_upload/%s' delimiter ';' CSV HEADER escape '\\' quote '"' """%(table,fn))
		...
		page = u"""
			<html>
			<head><title>Upload Q</title></head>
			<body>
			<h1>Upload file csv cho bo dem beta Q </h1>
			<h1>Chu y :upload file csv cho bo dem beta AC <a href='https://172.16.29.6/wsgi/upload_files_omnivore_z.py'>Tai day</a></h1>
