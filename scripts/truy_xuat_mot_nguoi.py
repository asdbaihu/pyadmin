# coding=utf-8
import pandas as pd
import psycopg2,logging,base64,os,sys
...
while True:
    conn = getConnection()
    cur = conn.cursor()
    logging.info(f"Please wait table name : {table_name} converting images")
    cur.execute(f"select id,img_data from {table_name} where img_data ilike '%%data:image%%' limit 10")
    rs = cur.fetchall()
    rows = iter(rs)
    conn.commit()
    cur.close()
    ...
    while True:
        try:
            ...
            cur.close()
            conn.close()
            logging.info(f"image id : {row[0]} of table :{table_name} has completed")
        except StopIteration:
            break

conn = getConnection()
cur = conn.cursor()
cur.execute(f"select id,agent,link,title,customize,variant_check,price_check,page_problem,price_detail,price,currency,condition,availability,cds_key,image,update_time from {table_name}")
...
logging.info(f"Convert images in table: {table_name} is completed")
