		...
		if 'file' in post:	
			...
			# #time = request.headers["time"]
				if fileitem.filename:
					...
					open('c:/wsgi_app/file_upload/'+ fn + account + '%s'%(table_name) , 'wb').write(fileitem.file.read())
...
no text,agent text, module text,domain text,title text,link text, quest text, answer text, time timestamp, process_time numeric,img_id text,
					...
					try:
						cur.copy_expert("""copy omnivore_%s_%s_%s(no ,agent , module ,domain ,title ,link , quest , answer , time , process_time,img_id ) from 'c:/wsgi_app/file_upload/%s%s%s' delimiter ';' CSV HEADER quote '"' """%(year,month,day,fn,account,table_name),sys.stdout)	
