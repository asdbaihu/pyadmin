...
no text,agent text, module text,domain text,title text,link text, quest text, answer text, time timestamp, process_time numeric,img_id text,
					...
					try:
						cur.copy_expert("""copy omnivore_%s_%s_%s(no ,agent , module ,domain ,title ,link , quest , answer , time , process_time,img_id ) from '/tmp/file_upload/%s%s%s' delimiter ';' CSV HEADER escape '\\' quote '"' """%(year,month,day,fn,account,table_name),sys.stdout)
		...
		cur.execute("update omnivore_%s_%s_%s set answer='' where answer='unselected'"%(year,month,day))							
