				...
				if 'table' in post:
					if post['table'] !='':
						...
						if len(query)> 0:
							...
							cur.execute("""select * from ("""  + que + """) as jotikaandmendaka where """ + """ and """.join(query) + """ order by """ + """,""".join(orderby) +""" """ + by + """ limit %s offset %s """%(display,start))
							rows = cur.fetchall()
							#except:
							#	rows_count=[0]
							#	rows = []
						...
						for i in range(len(row)):
							...
							for j in range(len(cols)):
								...
								elif type(row[i][j]).__name__ == 'dict':
									d[cols[j]] = json.dumps(row[i][j], ensure_ascii=False).encode('utf8').decode('utf-8')
								else:
									d[cols[j]] = row[i][j]
